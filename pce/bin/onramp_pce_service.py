...
def _updatessl():
    """Update OnRamp PCE server SSL key and certificate.
    
    If either the key or certificate arguments are not given, the respective
    file will be generated by openssl.

    Usage: onramp_pce_service.py updatessl [-h] [-k KEY] [-c CERT]

    optional arguments:
      -h, --help     show this help message and exit
      -k, --key      SSL key file to be used
      -c, --cert     SSL certificate file to be used
    """
    descrip = (
        'Update OnRamp PCE server SSL key and certificate. '
        'If either the key or certificate arguments are not given, the '
        'respective file will be generated by openssl.'
    )
    parser = argparse.ArgumentParser(prog='onramp_pce_service.py updatessl',
                                     description=descrip)
    parser.add_argument('-k', '--key', help='SSL key file to be used')
    parser.add_argument('-c', '--cert', help='SSL certificate file to be used')
    args = parser.parse_args(args=sys.argv[2:])
    ...
    if not ret_code == 0 or not os.path.isfile(pce_key_file):
        sys.exit('SSL key configuration failed.')
    ...
    if not ret_code == 0 or not os.path.isfile(pce_cert_file):
        sys.exit('SSL certificate configuration failed.')
